...
const { checkRememberMe, matric_no_isValid } = require('./utils');
...
router.post('/register', function (req, res) {
  ...
  const renderDetails = {
    name, email, username, password, password2, gender, matric_no
  }

  //Validation with express-validator
  req.checkBody('name', 'Name field cant be empty!').notEmpty();
  req.checkBody('email', 'Email field cant be empty!').notEmpty();
  req.checkBody('matric_no', 'Matric number field cannot be empty').notEmpty();
  ...
  if (errors) {
    ...
    if (!matric_no_isValid(matric_no)){
      res.render('register', {
        nameerror: "Invalid matric number",
        name, email, username, password, password2, gender, matric_no
      })
    }
    else {
    ...
    .then(founduser => {
      if (!founduser) {
        User.findOne({matric_no: req.body.matric_no})
        .then(matricexist => {
          if (!matricexist){
            ...
            .then(emailexists => {
              if (!emailexists){
                  ...
                  bcrypt.hash(newUser.password, salt, function (err, hash) {
                    ...
                    User.create(newUser, function (error, docs) {
                      if (error) {
                        ...
                        Notification.create({
                          userId: docs._id,
                          message: `${newUser.name}, update your address & profile picture so students can recognize you`,
                ...
                res.render('register', {
                  nameerror: "student with email already exists",
                  name, email, username, password, password2, gender, matric_no
                })
              }
            })
            ...
            res.render('register', {
              nameerror: "student with matric number already exists",
              name, email, username, password, password2, gender, matric_no
            })
          }
        })
        ...
        res.render('register', {
          nameerror: "student with username already exists",
          name, email, username, password, password2, gender, matric_no
        })
      }
    })
...
passport.use(new LocalStrategy({
  ...
  (username, password, done) => {
    User.findOne({ matric_no: username.trim() }, (err, user) => {
      if (err) {
        return done(err);
      }
      if (!user) {
        return done(null, false, { message: "invalid matric no" })
      }
      else {
...
router.post('/login',
  passport.authenticate('local', {
    ...
    failureFlash: 'Invalid matric number or password',
  }), (req, res, next) => {
